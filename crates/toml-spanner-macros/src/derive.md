...
Deserialization tries each variant in declaration order until one succeeds.
Errors from failed attempts are discarded.
...
Including helpers like `parse_string`, `display`, and `flatten_any` which
