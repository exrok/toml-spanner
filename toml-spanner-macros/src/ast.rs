    ...
    match key.as_str() {
        ...
        "from" => {
            if target.from_type.is_some() {
                Error::span_msg("Duplicate from attribute", attr.span())
            }
            if value.is_empty() {
                Error::span_msg("Expected a type for from", attr.span())
            }
        ...
        "try_from" => {
            if target.try_from_type.is_some() {
                Error::span_msg("Duplicate try_from attribute", attr.span())
            }
            if value.is_empty() {
                Error::span_msg("Expected a type for try_from", attr.span())
            }
        ...
        "deny_unknown_fields" => {
            if target.deny_unknown_fields {
                Error::span_msg("Duplicate deny_unknown_fields attribute", attr.span())
            }
    ...
    let offset = match name.as_str() {
        ...
        "style" => {
            let Some(TokenTree::Ident(style_ident)) = value.pop() else {
                Error::span_msg(
                    "Expected a style name (Header, Inline, Dotted, Implicit)",
                    ident.span(),
                )
            };
            if !value.is_empty() {
                Error::span_msg("Expected a single style name", ident.span())
            }
            let style_name = style_ident.to_string();
            match style_name.as_str() {
                "Header" | "Inline" | "Dotted" | "Implicit" => (),
                _ => Error::span_msg(
                    "Unknown style, expected Header, Inline, Dotted, or Implicit",
                    style_ident.span(),
                ),
            }
            trait_set &= TO_TOML;
