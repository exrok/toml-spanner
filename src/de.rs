...
/// Generates a [`FromToml`] implementation for a struct using a
/// single-pass iterate-and-match approach.
///
/// This is faster than [`TableHelper`] for tables with many fields because
/// it avoids per-field hash lookups — instead, it iterates over all entries
/// once and dispatches via a `match` on the key name.
///
/// # Syntax
///
/// ```ignore
...
///         optional source: SourceId,
///         optional "dev-dependencies" dev_deps: Vec<String>,
///         default tags: Vec<String>,
///     }
/// }
/// ```
///
/// To reject unknown fields, add `#[deny_unknown_fields]`:
///
/// ```ignore
/// toml_spanner::deserialize_table! {
///     #[deny_unknown_fields]
...
/// # Field kinds
///
/// - **`required`** — The struct field type is `T`. An error is pushed if
///   the key is absent from the input.
/// - **`optional`** — The struct field type is `Option<T>`. Missing keys
///   produce `None`.
/// - **`default`** — The struct field type is `T: Default`. Missing keys
///   produce `T::default()`.
///
/// An optional string literal before the field name provides a TOML key
/// rename. Without it, `stringify!(field_name)` is used as the key.
///
/// # Unknown fields
///
/// Without `#[deny_unknown_fields]`, unrecognized keys are silently ignored.
/// With it, all unknown keys are collected and reported as a single
/// [`ErrorKind::UnexpectedKeys`](crate::ErrorKind::UnexpectedKeys) error.
///
/// # Flatten
///
/// A `flatten` clause after the field list collects unrecognized keys into
/// a `Default`-initialized field via a closure that receives each unknown
/// key and value:
///
/// ```ignore
/// toml_spanner::deserialize_table! {
...
///     }
///     flatten extras: BTreeMap<String, String> = |key, value| {
///         extras.insert(key.name.to_owned(), value.as_str().unwrap().to_owned());
///     }
/// }
/// ```
#[macro_export]
macro_rules! deserialize_table {
    ...
    (
        ...
        impl<'de> $crate::FromToml<'de> for $struct_name {
            fn from_toml(
                __ctx: &mut $crate::Context<'de>,
                __item: &$crate::Item<'de>,
            ) -> Result<Self, $crate::Failed> {
                let __table = __item.expect_table(__ctx)?;
                $(let mut $field_name = None::<$field_type>;)*
                for (__key, __value) in __table {
                    match __key.name {
                        $(
                            $crate::deserialize_table!(@key $($toml_key,)? $field_name) => {
                                $crate::deserialize_table!(@deserialize __ctx __value $field_name : $field_type $(= |$map_item| $map_body)?)
                            }
                        )*
                        _ => {}
                    }
                }
                $($crate::deserialize_table!(@unwrap $field_kind $field_name __ctx __item);)*
                Ok($struct_name { $($field_name,)* })
            }
        }
    };
    ...
    (
        ...
        impl<'de> $crate::FromToml<'de> for $struct_name {
            fn from_toml(
                ...
                let __table = __item.expect_table(__ctx)?;
                $(let mut $field_name = None::<$field_type>;)*
                let mut __unexpected: Vec<(String, $crate::Span)> = Vec::new();
                for (__key, __value) in __table {
                    match __key.name {
                        $(
                            $crate::deserialize_table!(@key $($toml_key,)? $field_name) => {
                        ...
                        _ => {
                            __unexpected.push((__key.name.to_owned(), __key.span));
                        }
                    }
                }
    ...
    // flatten unknown fields into a catch-all
    (
        struct $struct_name:ident {
            $($field_kind:ident $($toml_key:literal)? $field_name:ident : $field_type:ty $(= |$map_item:ident| $map_body:block)?),* $(,)?
        }
        ...
        impl<'de> $crate::FromToml<'de> for $struct_name {
            fn from_toml(
                ...
                let __table = __item.expect_table(__ctx)?;
                $(let mut $field_name = None::<$field_type>;)*
                let mut $flat_name = <$flat_type as Default>::default();
                for (__key, __value) in __table {
                    match __key.name {
                        $(
                            $crate::deserialize_table!(@key $($toml_key,)? $field_name) => {
                ...
                $($crate::deserialize_table!(@unwrap $field_kind $field_name __ctx __item);)*
                Ok($struct_name { $($field_name,)* $flat_name, })
            }
        }
    };

    // internal: resolve TOML key name
    (@key $toml_key:literal, $field_name:ident) => { $toml_key };
    (@key $field_name:ident) => { stringify!($field_name) };

    // internal: deserialize a field value
    (@deserialize $ctx:ident $value:ident $field_name:ident : $field_type:ty = |$map_item:ident| $map_body:block) => {{
        let __map_fn: fn(&$crate::Item<'de>) -> ::core::result::Result<$field_type, $crate::Error> = |$map_item| $map_body;
        match __map_fn($value) {
            Ok(__val) => { $field_name = Some(__val); }
            Err(__err) => { $ctx.errors.push(__err); }
        }
    }};
    (@deserialize $ctx:ident $value:ident $field_name:ident : $field_type:ty) => {{
        $field_name = Some(
            <$field_type as $crate::FromToml>::from_toml($ctx, $value)?
        );
    }};

    // internal: unwrap fields before struct construction
    (@unwrap required $field_name:ident $ctx:ident $item:ident) => {
    ...
    (@unwrap optional $field_name:ident $ctx:ident $item:ident) => {};
    (@unwrap default $field_name:ident $ctx:ident $item:ident) => {
